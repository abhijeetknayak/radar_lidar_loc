...
def create_map(vr_time, vl_time, vr_loc, vl_loc, gt_pose_file, extrinsics_dir):
    ...
    map = None
    for idx in tqdm(range(0, len(gt_poses_vl), 1)):
        # Process Velodyne
...
if __name__ == '__main__':
    ...
    plot = False
    if plot:
        poses = interpolate_vo_poses(f'{oxford_data_dir}/gt/radar_odometry.csv', timestamps, timestamps[0])
    ...
    if construct_map:
        ...
        map_file = 'map_generation/d1.npz'
    ...
    if visualize:
        ...
        d200 = map[0:map.shape[0]:200]
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(downsampled_map[:, 0:3])
        pcd1 = o3d.geometry.PointCloud()
        pcd1.points = o3d.utility.Vector3dVector(d200[:, 0:3])

        if viz_ply:
            pcd = o3d.io.read_point_cloud("map_generation/downsampled_map.pcd")
            ipdb.set_trace()
            map = np.array(pcd.points)
            # pcd = o3d.io.read_point_cloud("robotcar_map.ply")

        if save_npz:
            np.savez("d50.npz", downsampled_map)
            np.savez("d200.npz", d200)
